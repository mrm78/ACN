...
export default function AddEvent(props) {
  ...
  const initialStates = {
    ...
    title: props.eve.title,
    caption: props.eve.description,
    time: props.eve.begin_time.slice(0,16),
    name: "",
  };
  ...
  const [image, setImage] = useState(props.eve.image?Const.baseUrl+props.eve.image:addimg);
  const [imageSize, setImageSize] = useState("100px");
  const [pic, setPic] = useState();
  const [imgPos, setImgPos] = useState("8%");
  ...
  async function handleMakeEvent(event) {
    ...
    if (isValid) {
      const data = {
        ...
        image: values.image,
        event_id: props.eve.id,
      };

      const formData = Const.toFormData(data);
      axios.post(`${Const.baseUrl}/community/edit_event`, formData,{
      headers : {Authorization: values.token}}
        ...
        if (res.status === 200) {
          if (res.data.status === "success") {
            ...
            } else if (res.data.error === "invalid event id") {
              setAlertMessage("The event does not exist.");
              setOpen(true);
          ...
          >
            ...
            >
              ...
              <Grid
                ...
                justify="center"
              >
              <Grid style={{ padding: "0px" }} sm={6} xs={8}>
                <Paper elevation={5} style={{width: "120px" }}>
              ...
              >
                <Grid item xs={12}>
                  ...
                  <form className={classes.container} noValidate>
                     <TextField
                       value={props.eve.begin_time.slice(0,16)}
                       id="datetime-local"
